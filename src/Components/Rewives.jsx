...
import { Star, StarOff, Edit3, MessageSquare, TrendingUp, Users, Loader2, Send, ThumbsUp, ThumbsDown, Reply, ChevronDown, ChevronUp, CheckCircle2, AlertCircle, X } from 'lucide-react';
...
const GeneralReviewSection = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.error('Unexpected error:', err);
      showAlert('Произошла ошибка при отправке', 'error');
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  const handleReaction = async (reviewId, reactionType) => {
    if (!currentUser) {
      showAlert('Войдите, чтобы оценить отзыв', 'error');
      return;
    }
  ...
  const handleReplySubmit = async (reviewId) => {
    if (!currentUser) {
      showAlert('Войдите, чтобы ответить', 'error');
      return;
    }
